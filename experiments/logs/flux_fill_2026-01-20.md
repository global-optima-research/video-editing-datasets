...
4. **高度依赖 prompt**: 生成质量与 prompt 强相关，不同场景需要不同 prompt，不适合自动化 pipeline
...
FLUX Fill **质量最好**，但**不适合自动化 pipeline**：

- 单帧修复效果优秀，颜色还原准确
- 184 帧视频处理需要 ~70 分钟，不适合大规模处理
- **致命问题**: 生成质量高度依赖 prompt，不同场景需要人工编写不同 prompt
- 适合人工监督的少量高质量修复，不适合全自动数据集构建
